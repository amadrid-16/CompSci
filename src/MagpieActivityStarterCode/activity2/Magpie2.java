package MagpieActivityStarterCode.activity2;
/**
 * A program to carry on conversations with a human user.
 * This is the initial version that:  
 * <ul><li>
 *       Uses indexOf to find strings
 * </li><li>
 * 		    Handles responding to simple words and phrases 
 * </li></ul>
 * This version uses a nested if to handle default responses.
 * @author Laurie White
 * @version April 2012
 */
public class Magpie2
{
	/**
	 * Get a default greeting 	
	 * @return a greeting
	 */
	public String getGreeting()
	{
		return "Hello, let's talk. What would you like to talk about?";
	}
	
	/**
	 * Gives a response to a user statement
	 * 
	 * @param statement
	 *            the user statement
	 * @return a response based on the rules given
	 */
	public String getResponse(String statement)
	{
		String response = "";
		
		if (statement.indexOf("mother") >= 0
				|| statement.indexOf("father") >= 0
				|| statement.indexOf("sister") >= 0
				|| statement.indexOf("brother") >= 0)
		{
			response = "Tell me more about your family.";
		}
		else if(statement.indexOf("dog") >= 0
				|| statement.indexOf("cat") >= 0){
			response = "Tell me more about your pet";       //Pet
		}
	      else if(statement.indexOf("Eno") >= 0              //Teacher Name
	            ||statement.indexOf("eno") >= 0){
	         response = "He sounds like a good teacher.";
	      }
	      else if(statement.indexOf("engineering") >= 0
	            ||statement.indexOf("Engineering") >= 0
	            ||statement.indexOf("Robotics") >= 0
	            ||statement.indexOf("robotics") >= 0
	            ||statement.indexOf("Math") >= 0
	            ||statement.indexOf("math") >= 0          //1 Keyword
	            ||statement.indexOf("Science") >= 0
	            ||statement.indexOf("science") >= 0
	            ||statement.indexOf("Tech") >= 0
	            ||statement.indexOf("tech") >= 0
	            ||statement.indexOf("Stem") >= 0
	            ||statement.indexOf("STEM") >= 0
	            ||statement.indexOf("stem") >= 0){
	         response = "So you are interested in STEM?";
	      }
	      else if(statement.indexOf("Angel")>= 0
	            ||statement.indexOf("angel")>= 0       //2nd Keyword
	            ||statement.indexOf("Name")>= 0
	            ||statement.indexOf("name")>= 0){
	         response = "My name is Angel";
	      }
	      else if(statement.indexOf("soccer")>=0
	    		||statement.indexOf("football")>=0
	    		||statement.indexOf("volleyball")>=0	//3rd keyword
	    		||statement.indexOf("baseball")>=0
	    		||statement.indexOf("sports")>=0
	    		||statement.indexOf("basketball")>=0){
	    	 response = "So you're into sports then?"; 	
	      }
	      else if(statement.trim().equals("")){
	         response = "Say something, please.";      //Empty Response
	      }
	      else if (statement.indexOf(" no ") >= 0
				||statement.indexOf(" no") >= 0
				||statement.indexOf("no") >= 0)
		  {
			response = "Why so negative?";
		  }
	      else
	      {
				response = getRandomResponse();
	      }
			return response;
	}

	/**
	 * Pick a default response to use if nothing else fits.
	 * @return a non-committal string
	 */
	private String getRandomResponse()
	{
		final int NUMBER_OF_RESPONSES = 4;
		double r = Math.random();
		int whichResponse = (int)(r * NUMBER_OF_RESPONSES);
		String response = "";
		
		if (whichResponse == 0)
		{
			response = "Interesting, tell me more.";
		}
		else if (whichResponse == 1)
		{
			response = "Hmmm.";
		}
		else if (whichResponse == 2)
		{
			response = "Do you really think so?";
		}
		else if (whichResponse == 3)
		{
			response = "You don't say.";
		}

		return response;
	}
}
